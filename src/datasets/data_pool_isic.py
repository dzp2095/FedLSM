...
from src.datasets.dataset_isic import SkinDatasetRaw, PesudoSubset, OriginialSubset, SrongAugmentedSubset
from src.datasets.sampler import TrainingSampler

class SkinDataPool:
    def __init__(self, cfg):
        self.cfg = cfg
        self.train_raw_dataset = SkinDatasetRaw(csv_file=self.cfg['dataset']['train'], cfg=self.cfg)
    ...
    def _get_data_idx(self, nets, device):
        ...
        with tqdm(total=n_data, desc=f'Calculate Entropy. ', unit='img') as pbar:
            for _, images, _ in dataloader:
                ...
                for net in nets:
                    with torch.no_grad():
                        output = F.softmax(net(images).detach(), dim = 1).cpu().detach().numpy()
                    entr_res = entropy(output, axis=1)
                    entr_res = torch.from_numpy(entr_res)
                    entr_res = entr_res.to(device=device, dtype=torch.float32)
                    ensemble_entropy = ensemble_entropy + entr_res
    ...
    def get_dataloader(self, nets, device, pseudo_batch_size, uncertain_batch_size):
        logging.info("Rerank Curriculum Data Pool")
